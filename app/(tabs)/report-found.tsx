...
import { createFoundItem } from "../../src/services/lostFoundService";
...
export default function ReportFoundScreen() {
  ...
  async function handleSubmit() {
    if (!user) return;
    if (!itemName.trim() || !description.trim() || !locationFound.trim() || !dateFound.trim()) {
    ...
    try {
      setLoading(true);
      await createFoundItem(user.uid, {
      ...
      showToast("Found report submitted for review.", "success");
    ...
    <Screen scrollable>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <View style={styles.header}>
          <View style={styles.iconBox}><Ionicons name="add-circle-outline" size={22} color={COLORS.primary} /></View>
          <View style={{ flex: 1 }}>
            <Text style={styles.title}>Report found item</Text>
            <Text style={styles.subtitle}>Admin approval prevents spam and duplicate posts.</Text>
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          />
          <AppTextInput label="Found location" placeholder="Location" value={locationFound} onChangeText={setLocationFound} />
          <AppTextInput label="Date found" placeholder="Date" value={dateFound} onChangeText={setDateFound} />
...
const styles = StyleSheet.create({
  ...
  iconBox: { width: 44, height: 44, borderRadius: 16, backgroundColor: COLORS.primaryLight, alignItems: "center", justifyContent: "center" },
  ...
  card: { backgroundColor: COLORS.card, borderRadius: RADIUS.lg, padding: SPACING.md, borderWidth: 1, borderColor: COLORS.border, marginBottom: 120 },
  ...
  chip: { marginRight: SPACING.sm, marginBottom: SPACING.sm, paddingHorizontal: SPACING.md, height: 32, borderRadius: 999, backgroundColor: COLORS.background, borderWidth: 1, borderColor: COLORS.border, alignItems: "center", justifyContent: "center" },
